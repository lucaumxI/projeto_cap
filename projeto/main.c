...
int main()
{
    ...
    {
    case 1:
        rank = fopen("rankf.bin", "rb");
        while(fread(&reg[cont],sizeof(struct registro),cont+1,rank)==cont+1)
        {
            cont++;
        }
        for(int i=0;i<cont+1;i++){
            printf("%s %f\n", reg[i].nome, reg[i].ponto);
        }
        reg[cont].ponto = facil();
        break;
    case 2:
        rank = fopen("rankm.bin", "rb");
        while(fread(&reg[cont],sizeof(struct registro),cont+1,rank)==cont+1)
        {
            cont++;
        }
    ...
    {
    case 1:
        rank = fopen("rankf.bin", "wb");
        break;
    case 2:
        rank = fopen("rankm.bin", "wb");
        break;
    case 3:
        rank = fopen("rankd.bin", "wb");
        break;
    }
    scanf("%s", reg[cont].nome);
    qsort(reg, cont+1, sizeof(struct registro), compara);
    fwrite(&reg,cont+1,sizeof(struct registro),rank);
    fread(&reg[cont],sizeof(struct registro),cont+1,rank);
    for(int i=0;i<cont+1;i++){
        printf("%s %f\n", reg[i].nome, reg[i].ponto);
    }
    fclose(rank);
